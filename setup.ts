...
const endpoint: string = process.env.RPC_URL || "https://api.devnet.solana.com"; 

export async function loadContractAndCallConstructor(name: string,space: number = 8192,programId:string,account:number[]):
    Promise<{ program: Program, payer: Keypair, provider: AnchorProvider, storage: Keypair, program_key: PublicKey }> {

    const {program, payer, provider, program_key} = await loadContract(name,programId,account);

    const storage = Keypair.generate();
    await create_account(storage, program_key, space);

    // await program.methods.new(payer.publicKey)
...
export async function loadContract(name: string,programId:string,account:number[]):
    ...
    const userKeypair = new Uint8Array(account);
    const payer = Keypair.fromSecretKey(userKeypair)
    const provider = AnchorProvider.env();
    const program_key = new PublicKey(programId);
    const program = new Program(idl, program_key, provider)
    return {program,payer, provider, program_key: program_key};
}
